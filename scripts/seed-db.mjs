...
async function seed() {
  ...
  console.log(
    `  Target: ${process.env.POSTGRES_URL.replace(/\/\/.*@/, "//***@")}\n`
  );
  ...
  try {
    ...
    // 3. Create Novel (is_featured + featured_order so it appears on the Boudoir hero)
    console.log("Creating novel...");
    const novelRes = await pool.query(
      `
      INSERT INTO novels (title, series_id, author_id, cover_image_url, synopsis, genre_tags, rating, rating_count, publication_date, is_featured, featured_order)
      VALUES (
    ...
    console.log(
      "\n✅ Database seeded successfully with 'Tactile Noir Luxury' content."
    );
    console.log("   - 1 author (Elias Thorne)");
    console.log("   - 1 series (The Obsidian Court)");
    console.log("   - 1 novel (Whispers in the Velvet Dark) — featured");
    console.log("   - 3 chapters (1 free, 2 locked)");
    console.log("   - 2 characters (Silas Vale, Elara Vance)");
  } catch (err) {
