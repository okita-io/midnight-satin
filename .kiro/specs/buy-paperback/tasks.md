...
- [ ] 1. Create pricing module and word count utilities
  - [ ] 1.1 Create `src/lib/paperback/pricing.ts` with `PricingConfig` interface, `DEFAULT_PRICING_CONFIG`, and `calculatePaperbackPrice` pure function
  ...
  - [ ] 1.2 Create `src/lib/paperback/word-count.ts` with `countWords` pure function and `getNovelWordCount` async function
  ...
  - [ ] 1.3 Write property tests for price calculation (Property 1)
  ...
  - [ ] 1.4 Write property tests for word count (Properties 2, 3)
  ...
  - [ ] 1.5 Write property test for page count estimation (Property 4)
...
- [ ] 3. Add data model and DB layer
  - [ ] 3.1 Add `PaperbackOrder` interface to `src/lib/db/types.ts`
  ...
  - [ ] 3.2 Add `paperback_orders` table DDL to `src/lib/db/schema.sql`
    - Include UNIQUE constraint on `stripe_session_id` for idempotency
    - Add indexes on `reader_id`, `novel_id`, `stripe_session_id`
    - _Requirements: 7.1_

  - [ ] 3.3 Create `src/lib/db/paperback-orders.ts` with `insertPaperbackOrder` function
...
- [ ] 4. Add Buy Paperback button to Novel Detail Header
  - [ ] 4.1 Modify `src/app/_components/novel-detail-header.tsx` to add a `menu_book` icon button
  ...
  - [ ]* 4.2 Write property test for navigation URL correctness (Property 5)
...
- [ ] 5. Implement Paperback Purchase Page (Coming Soon state)
  - [ ] 5.1 Create `src/app/novel/[novelId]/paperback/page.tsx` as a Server Component
  ...
  - [ ] 5.2 Implement Coming Soon state UI within the page
    - Conform to the Tactile Noir Luxury design system used across the app — reuse existing Tailwind classes (`bg-void`, `bg-surface`, `text-primary`, `btn-gold`, `font-playfair`, `font-cinzel`, `font-literata`, `font-marcellus`) and component patterns from other pages (e.g., Novel Detail, Cast Gallery)
    ...
    - "Back to Novel" button — reuse `btn-gold` class from existing pages (gold background, void text, uppercase tracking)
    - Back arrow in header — reuse the same back button component/styling from Novel Detail Header
    - Void black background, surface card styling, gold accents
    - Reference `reference/midnight_satin_prd.html` for design tokens and `reference/the_novel_detail.html` for layout patterns
    - _Requirements: 2.3, 2.4, 2.5, 2.6, 2.7, 2.8_

  - [ ] 5.3 Write property test for feature flag determines page state (Property 6)
  ...
  - [ ] 5.4 Write property test for Coming Soon state renders novel data (Property 7)
...
- [ ] 6. Implement Active Purchase State (client component)
  - [ ] 6.1 Create `src/app/novel/[novelId]/paperback/paperback-client.tsx` as a Client Component
    - Conform to the app's existing look and feel — reuse Tailwind utility classes, font families, color tokens, and component patterns already established in other pages
    ...
    - Login prompt if unauthenticated — reuse existing auth prompt patterns and link styling (with return URL to `/novel/${novelId}/paperback`)
    - Call `createPaperbackCheckout` server action on button click, then `window.location.href` redirect to Stripe
    - Display error message from server action if checkout fails — use existing toast/alert patterns if available
    - _Requirements: 10.1, 10.2, 10.3, 10.4, 10.5, 10.6, 10.7_

  - [ ] 6.2 Write property test for active state displays formatted price (Property 15)
  ...
  - [ ] 6.3 Write property test for login prompt includes return URL (Property 16)
...
- [ ] 8. Implement server action for Stripe Checkout
  - [ ] 8.1 Create `src/app/actions/paperback.ts` with `createPaperbackCheckout` server action
  ...
  - [ ] 8.2 Write property test for checkout session params (Property 8)
  ...
  - [ ] 8.3 Write property test for error message sanitization (Property 9)
...
- [ ] 9. Implement Stripe webhook handler
  - [ ] 9.1 Create `src/app/api/webhooks/stripe/route.ts` with POST handler
  ...
  - [ ] 9.2 Write property test for webhook idempotency (Property 11)
  ...
  - [ ] 9.3 Write property test for webhook rejects invalid signatures (Property 12)
...
- [ ] 10. Implement Success Page
  - [ ] 10.1 Create `src/app/novel/[novelId]/paperback/success/page.tsx` as a Server Component
  ...
  - [ ] 10.2 Write property test for success page redirects on invalid session (Property 14)
...
- [ ] 11. Update environment configuration
  - [ ] 11.1 Update `.env.example` to document new environment variables
